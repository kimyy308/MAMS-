#!/bin/sh
# Updated, 19-Jan-2021 by Yong-Yub Kim (If old -> 1 day run)
...
nowdate=`date --date="2 days ago" +%Y%m%d`
if [ $fulldate -eq $nowdate ] ; then   
ntimes=`echo "${yearday}*${oneday}+${oneday}*3"|bc`
else  # for Auto_old
ntimes=`echo "${yearday}*${oneday}"|bc`
fi
...
txyear=2019
txday=0
#endtxyear= `echo "${year} " |bc`

for (( txyear; txyear<${year}; txyear++ )) do
  totalyearday=`date -d ${txyear}1231 +%j`
  txday=`echo "${txday}+${totalyearday}"|bc`
done
...
if [ $fulldate -eq ${year}0101 ]; then
  $MAMS/App/runmatlab_auto $fulldate $MAMS/Source/MATLAB/preprocessing/set_time_ini.m
fi
