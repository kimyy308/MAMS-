...
# Modified, 25-Mar-2021 by Jihun Jung (Download path)
...
cd $fulldate
  ...
  for (( arr_j=0; arr_j<=3; arr_j++ )) do
    for (( arr_g=0; arr_g<=1; arr_g++ )) do
      ...
      wget "http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gdas."${fulldate}/${arr_i[${arr_j}]}/atmos/${filename}
      wget "http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gdas."${fulldate}/${arr_i[${arr_j}]}/atmos/${filename}.idx
    done
  done
  ...
  if [[ -f "$filename" ]]; then
    ...
    for (( arr_j=0; arr_j<=7; arr_j++ )) do
      ...
      if [[ "$filesize" -gt 250000000 ]]; then
        ...
        wget "http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs."${fulldate}/00/atmos/${filename}
        wget "http://nomads.ncep.noaa.gov/pub/data/nccf/com/gfs/prod/gfs."${fulldate}/00/atmos/${filename}.idx
    done
