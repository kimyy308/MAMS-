...
##    cd ${workdir}${year}
    ...
    indthree=`printf "%04d" $ind`
  	/data1/auto/App/kwgrib2/kwgrib -d ${indthree} wnd10m.gdas.${year}${monthtwo}.grib2 -netcdf ${tempdir}Uwnd10m.gdas.${year}${monthtwo}_${indthree}.nc
    done
    ...
    indthree=`printf "%04d" $ind`
  	/data1/auto/App/kwgrib2/kwgrib -d ${indthree} wnd10m.gdas.${year}${monthtwo}.grib2 -netcdf ${tempdir}Vwnd10m.gdas.${year}${monthtwo}_${indthree}.nc
    done 
...
##    indthree=`printf "%04d" $ind`
##  		rm -f ./temp/Uwnd10m.gdas.${year}${monthtwo}_${indthree}.nc
##    done
##    for (( ind=14; ind<=${lastindex}; ind+=14 ))do
##    indthree=`printf "%04d" $ind`
##  		rm -f ./temp/Vwnd10m.gdas.${year}${monthtwo}_${indthree}.nc
##
##    done

##        ncecat ./pressfc/pressfc.gdas.${year}${monthtwo}* -O ./pressfc.gdas.${year}${monthtwo}.nc
        ncrcat -h ${tempdir}Uwnd10m.gdas.${year}${monthtwo}* -O /data1/stlee/ext_hdd/NOMADS/${year}/nc_file/Uwnd10m.gdas.${year}${monthtwo}.nc
        ncrcat -h ${tempdir}Vwnd10m.gdas.${year}${monthtwo}* -O /data1/stlee/ext_hdd/NOMADS/${year}/nc_file/Vwnd10m.gdas.${year}${monthtwo}.nc

    rm -f ${tempdir}Uwnd10m.gdas.${year}${monthtwo}*
    rm -f ${tempdir}Vwnd10m.gdas.${year}${monthtwo}*
