...
source /home/auto/.bashrc
  ...
  while [ ${this_time} -le ${last_time} ]; do
    ...
    dstr_time=`date -d "${first_time} ${idx} day" +%Y-%m-%d`
    this_time=`date -d "${first_time} ${idx} day" +%Y%m%d`
    this_year=`echo $this_time | cut -c1-4`
    this_month=`echo $this_time | cut -c5-6`
    this_day=`echo $this_time | cut -c7-8`
    data_dir=${work_dir}/Data/OSTIA/daily/${this_year}/${this_month}
    filename=${data_dir}/OSTIA_${this_time}.nc
    mkdir -p ${data_dir}
