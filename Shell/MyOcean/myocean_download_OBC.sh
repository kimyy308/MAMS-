...
for (( lag_day=0; lag_day<=lag_max; lag_day++ )) do
  ...
  case ${bdy_idx} in
    ...
    then
       ...
       conda activate
       python -m motuclient --user ykim7 --pwd Mepl036$ --motu http://nrt.cmems-du.eu/motu-web/Motu -s GLOBAL_ANALYSIS_FORECAST_PHY_001_024-TDS -d global-analysis-forecast-phy-001-024 -x ${x_left} -X ${x_right} -y ${y_lower} -Y ${y_upper} -t "${dstr_time}" -T "${dstr_time}" -z ${z_min} -Z ${z_max} ${var_str} --out-dir ./ --out-name ${data_dir}/myocean_${bdry_name}_${this_time}.nc
       conda deactivate
       echo ""
