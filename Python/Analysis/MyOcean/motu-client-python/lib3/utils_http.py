...
def open_url(url, **kargsParam):
    ...
    if kargs.has_key('proxy'):
        ...
        handlers.append( http.client.ProxyHandler({'http':urlProxy, 'https':urlProxy}) )
        handlers.append( http.client.HTTPBasicAuthHandler() )
        
        del kargs['proxy']
        
    if kargs.has_key('authentication'):
        # create the password manager
        password_mgr = http.client.HTTPPasswordMgrWithDefaultRealm(ssl_version=ssl.PROTOCOL_TLSv1)
        ...
        handlers.append(http.client.TLS1Handler(password_mgr))
    ...
    _opener = http.client.build_opener(*handlers)
    ...
    # create the request
    if( data != None ):
        r = http.client.Request(url, data, **kargs)
    else:
        r = http.client.Request(url, **kargs)
