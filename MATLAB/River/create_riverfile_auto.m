...
% Updated, 01-Feb-2021 by Yong-Yub Kim (using OSTIA temperature)
% % clear all
% % close all
% clc
...
OSTIAdir='/home/auto/MAMS/Data/OSTIA';
OSTIApath=[OSTIAdir,'/daily/',yearstr, '/', monthstr];
OSTIA_name=[OSTIApath,'/','OSTIA_', tempday, '.nc'];
...
% end
riv_raw = textread(['/home/auto/MAMS/Data/CJD/' num2str(year) '/datong_archive_', ...
...
% edited by e.b.cho for interpolation to days
dis_total_days = sum(eomday(year,[1:12]));

r = 2;
% [Dec Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec Jan] for interpolation
season_discharge=[440 378  296  299  209  216  271  907  1481  1345  1084  619  440  378]; 

% edited by e.b.cho 2020.01.9 
daily_discharge=interp1(seasonal_day,season_discharge,1:dis_total_days);
% original
% daily_discharge=interp1(seasonal_day,season_discharge,1:365);

% edit by e.b.cho 2021.01.05 because of error on last day of year 
if fulltime == dis_total_days
dis = daily_discharge(1);
else
dis=daily_discharge(fulltime+1);
end
% original 
...
% % Tuman river temp(OSTIA) -> [1.51, 1.09, 1.68, 3.90, 7.87, 13.00, 17.54, 20.49, 18.91, 14.65, 9.37, 4.32]
...
OSTIA_lat = ncread(OSTIA_name,'lat');
OSTIA_lon = ncread(OSTIA_name,'lon');
%sst_s = ncread(OSTIA_name, 'analysed_sst');
%sst_e = ncread(OSTIA_name, 'analysis_error');
sst_s = ncread(OSTIA_name, 'analysed_sst');
sst_e = ncread(OSTIA_name, 'analysis_error');
...
[indw, inde, inds, indn]=findind_Y(1/4, [115, 162, 15, 52], OSTIA_lon, OSTIA_lat);
    ...
    for i=indw:inde
        for j=inds:indn
            if (isnan(sst_s(i,j))~=1)
                River.dist(k,i,j)=m_lldist([River.lon(k) OSTIA_lon(i)], [River.lat(k) OSTIA_lat(j)]);
            else
