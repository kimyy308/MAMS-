...
%%    dropboxpath='/home/kimyy/Dropbox'; %% DAMO server
    dropboxpath='/HDD1/kimyy/Dropbox'; %% DGIST 3rd
    addpath(genpath([dropboxpath '/source/matlab/Common/m_map']));
    addpath(genpath([dropboxpath '/source/matlab/Common/Figure']));
...
% % addpath(genpath(['/HDD1/kimyy/Model/ROMS/roms_nwp/nwp_1_10/output/',testname,'/TRANSP/m_map']))
% % addpath(genpath(['/HDD1/kimyy/Model/ROMS/roms_nwp/nwp_1_10/output/',testname,'/TRANSP/netcdf_old']))

fid=fopen('/HDD1/kimyy/Model/ROMS/roms_nwp/nwp_1_10/output/modelinfo');
...
%%fid=fopen('/HDD1/kimyy/Model/ROMS/roms_nwp/nwp_1_10/yearfile.in');
fid=fopen('/HDD1/kimyy/reanalysis_data/output/exp01/tryearfile.in');
yearinfo=textscan(fid,'%s');
fclose(fid);

%yearstart = str2num(yearinfo{1,1}{1,1})-1;  %% get present year
%yearend = str2num(yearinfo{1,1}{1,1})-1;
yearstart = str2num(yearinfo{1,1}{1,1});  %% get present year
yearend = str2num(yearinfo{1,1}{1,1});
%yearstart = 1983;  %% get present year
%yearend = 1983;

foot = '.nc';
% inputdir = strcat('/data1/kimyy/Model/ROMS/roms_nwp/nwp_1_10/output/',testname,'/'); %% for DAMO
% outputdir = '/data1/kimyy/Model/ROMS/roms_nwp/nwp_1_10/output/TRANSP/';  %% for DAMO
inputdir = ['/HDD1/kimyy/reanalysis_data/output/exp01/', num2str(yearstart, '%04i'), '/']; %% for COCO, DA
outputdir = inputdir; 

outfile = strcat(outputdir,'nwp_1_10_monthly_',testname,'_',num2str(yearstart,'%04i'),'.txt');

fid = fopen(outfile,'w+');
fprintf(fid,'%%korea  tsugaru   soya   taiwan kuro_intru  yellowsea  western  eastern  \n');
fclose(fid);

transind=1;
for year = yearstart:yearend
    for month=1:12
%     prefix = '/HDD1/kimyy/Model/ROMS/roms_nwp/nwp_1_10/output/';
...
%               142.0  47.0 142.04  45.0 ];
    point_name={'korea','tsugaru','soya','taiwan','kuro_intrusion','yellowsea','western','eastern'};
    kts = [128.0 35.2 134.0 35.2 ...  
    ...
    128.5  35.0 129.3 34.4 ...
    129.3  34.4 130.5 33.5];
