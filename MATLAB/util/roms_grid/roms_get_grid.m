function grd = roms_get_grid(Vtransform,Vstretching,grd_file,scoord,tindex,calc_zuv)
% vert_param;
...
z_r=zlevs(Vtransform,Vstretching,h,0.*h,theta_s,theta_b,hc,N,'r');  
z_w=zlevs(Vtransform,Vstretching,h,0.*h,theta_s,theta_b,hc,N,'w');  
grd.z_r = reshape(z_r,[N size(h)]);
grd.z_w = reshape(z_w,[Np size(h)]);
