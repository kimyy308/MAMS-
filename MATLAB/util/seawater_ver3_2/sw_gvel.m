...
lf=lf(:)';
LF = lf(ones(m,1),:);
if diff(lon(1:2))>0  & abs(diff(lon(1:2))) > abs(diff(lat(1:2)))
vel   = -( ga(:,2:n)-ga(:,1:n-1) ) ./ LF;
elseif diff(lat(1:2))>0  & abs(diff(lon(1:2))) < abs(diff(lat(1:2)))
vel   = ( ga(:,2:n)-ga(:,1:n-1) ) ./ LF;
elseif diff(lon(1:2))<0  & abs(diff(lon(1:2))) > abs(diff(lat(1:2)))
vel   = ( ga(:,2:n)-ga(:,1:n-1) ) ./ LF;
elseif diff(lat(1:2))<0  & abs(diff(lon(1:2))) > abs(diff(lat(1:2)))
vel   = -( ga(:,2:n)-ga(:,1:n-1) ) ./ LF;
end
