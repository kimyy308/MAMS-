            ...
            for hourind = 1:4
                ...
                if (dayind==1 && hourind ==1)
                    ...
                    [UM_lon_min, UM_lon_max, UM_lat_min, UM_lat_max] =  ...
                        ...
                        UM_lon, UM_lat);
                    if isvector(UM_lon) && isvector(UM_lat)
                        [UM_lat, UM_lon] = meshgrid(UM_lat, UM_lon);
                    end
                end
...
for timei=1:length(UM_data_cut(1,1,:))
    ...
    data_interpolated(:,:,timei) = tempdata;
end
...
for i=1:length(UM_data_cut(1,1,:))
    ...
    netcdf.putVar(ncid, dvarid, [0 0 i-1], [data_info.Size(1) data_info.Size(2) 1], data_interpolated(:,:,i)); %%[x y t], Z'(x,y)
end
