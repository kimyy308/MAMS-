...
% Check dimensions
%
% save 'E:\Data\Model\ROMS\nwp_1_20\input\test39\abc.mat' %%Y.Y.Kim
...
if (sum(sum(isdata))==0) 
%  disp('no data')
  disp(['GET_MISSING_VAL: all the data point is missing (usually deep area)-> using default value:',...
       num2str(default)])
...
%                          lon(ismask),lat(ismask),'nearest', {'QJ'});
  field(ismask)=griddata(lon(~ismask),lat(~ismask),field(~ismask),...
