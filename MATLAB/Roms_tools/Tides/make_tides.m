...
%  Updated   11-May-2018 by Yong-Yub Kim (cp grdfile -> tidefile)
%  Updated   07-Jul-2018 by Yong-Yub Kim (sensoring operation system)
...
for yearind=1976:2005
    frcname=['/data1/kimyy/Model/ROMS/roms_nwp/nwp_1_20/input/test51/nwp_1_20_',num2str(yearind,'%04i'),'_tides.nc'];
    ...
    comp=computer;
    if (comp=='GLNXA64')  %% LINUX
            system(['cp ',grdname,' ',frcname])  %% make a tide file from copied grd file
    elseif (comp=='WIN64')  %% WIN
        system(['copy ',grdname,' ',frcname])  %% make a tide file from copied grd file
    else
        'cannot recognize operation type'
    end
    ...
    if (exist('tide_frc_name')==1)
        ...
        nc_add_tides_Y(frcname,Ntides,date_mjd,components)
