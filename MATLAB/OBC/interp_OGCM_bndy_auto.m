function interp_OGCM_bndy_auto(OGCM_path_day, Roa, interp_method, ...
        OGCM, tndx_OGCM, ROMS)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Updated 11-Jun-2019 by Y.Y.Kim
...
OGCM_path = [OGCM_path_day(1:end-9), presentday.all, '/'];
if (exist(OGCM_path, 'dir')~=7)
    OGCM_path = [OGCM_path_day(1:end-9), presentday.all, '_forecast/'];
end
%

disp(['  Horizontal interpolation: ',datestr(today.num+tndx_OGCM-1) ])
...
for diri=1:4
    OGCM.data(diri).filename = [OGCM_path, 'myocean_', ROMS.data(diri).direction, '_', presentday.all, '.nc'];
...
for diri=1:4
    if ~isempty(ROMS.brync)
        for vari=[ind_zeta,10,11]
            ROMS.brync{[ROMS.var(vari).name, '_', ROMS.data(diri).direction]}(tndx_OGCM,:) = ...
